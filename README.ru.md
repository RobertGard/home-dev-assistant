...
2. Диспетчер создаёт **отдельную сессию OpenCode** с **read-only verifier-агентом** (`edit: deny, bash: allow`)
3. Verifier выполняет 4 обязательных проверки:
   - **Здоровье кода и сборки**: lint, typecheck, тесты, `git diff`
   - **Логи приложения**: `docker logs --tail 100` для каждого контейнера, log-файлы
   - **Поведение в рантайме**: curl-проверки API, взаимодействие с браузером через Playwright (консоль, сетевые ошибки, содержимое страниц)
   - **Покритериальная проверка**: каждый критерий приёмки сверяется с доказательствами
4. Каждая проверка **требует фактический вывод как доказательство** — агент не может пропустить или сфабриковать
5. **DeepSeek AI judge** оценивает отчёт верификации и возвращает вердикт PASSED/FAILED
6. При FAILED → авто-создаётся задача-фикс с **теми же критериями `--verify`** (критерии копируются)
7. Если задача-фикс тоже не проходит → создаётся ещё одна, и так до успешной верификации
...
9 специализированных агентов с ролевыми разрешениями:
...
1. `bash ./scripts/setup-stack.sh` → укажите `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`
...
- **Telegram не работает:** убедитесь что `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` и `N8N_API_KEY` заданы в `.env`, затем `bash ./scripts/bootstrap-telegram-integration.sh`
