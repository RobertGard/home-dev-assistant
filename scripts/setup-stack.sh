...
ask_yes_no() {
  ...
  while true; do
    printf '%s [д/н, по умолчанию %s]: ' "$prompt" "$shown_default"
    IFS= read -r value
    value="$(trim "$value")"
...
if [ -f "$ENV_FILE" ]; then
  if ! ask_yes_no "Файл .env уже существует. Перезаписать его?" n; then
    printf 'Оставляю текущую конфигурацию без изменений.\n'
    exit 0
  fi
fi
...
for ((i = 1; i <= WORKER_COUNT; i++)); do
  ...
  if [ "$i" -eq 1 ]; then
    repo_slug="ai-backend-marketing-agent"
    repo_url="https://github.com/sixmo-team/ai-backend-marketing-agent.git"
    repo_path="ai-backend-marketing-agent"
    turbo_enabled="true"
